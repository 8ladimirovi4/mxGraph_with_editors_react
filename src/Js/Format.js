...
Format.prototype.roundableShapes = [
    'label', 'rectangle', 'parallelogram', 'swimlane', 'table', 'chart', 'triangle', 'link', 'rhombus', 'hexagon', 'singleArrow', 'doubleArrow', 'flexArrow'
];
Format.prototype.notFillableShapes = [
    'arc'
];
...
Format.prototype.initSelectionState = function()
{
    return {vertices: [], edges: [], x: null, y: null, width: null, height: null, style: {},
        containsImage: false, containsLabel: false, fill: true, rounded: true,
        autoSize: false, image: true, shadow: true, lineJumps: true};
};
...
Format.prototype.updateSelectionStateForCell = function(result, cell, cells)
{
    ...
    if (state != null)
    {
        ...
        result.containsImage = result.containsImage || shape == 'image' || shape == 'bmrz';
...
{
    return (state.view.graph.model.isVertex(state.cell) && mxUtils.indexOf(this.notFillableShapes, mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null)) < 0) ||
...
{
    var shape = mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null);
    return (shape == 'label' || shape == 'image' || shape == 'bmrz');
};
Format.prototype.isShadowState = function(state)
{
    var shape = mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null);    
    return (shape != 'image' && shape != 'bmrz');
};
...
Format.prototype.refresh = function()
{
    ...
    if (!this.container || this.container.style.width == '0px')
        return;
...
BaseFormatPanel.prototype.createColorOption = function(label, getColorFn, setColorFn, defaultColor, listener, callbackFn, hideCheckbox)
{
    ...
    var apply = function(color, disableUpdate, forceUpdate)
    {
        if (!applying)
        {
            ...
            // Fine-tuning in Firefox and IE8 standards
            if (document.documentMode == 8)
...
BaseFormatPanel.prototype.addArrow = function(elt, height)
{
    ...
    arrow.innerHTML = '<img border="0" src="' + 'data:image/png;base64,'
...
ArrangePanel.prototype.init = function()
{
    ...
    if ((!ss.containsLabel || ss.edges.length == 0) && ss.style.rotatable)
    {
...
ArrangePanel.prototype.addAlign = function(div)
{
    ...
    var left = this.editorUi.toolbar.addButton('geSprite-alignleft', mxResources.get('leftAlign'),
        function() { graph.alignCells(mxConstants.ALIGN_LEFT); }, stylePanel);
    var center = this.editorUi.toolbar.addButton('geSprite-aligncenter', mxResources.get('centerAlign'),
        function() { graph.alignCells(mxConstants.ALIGN_CENTER); }, stylePanel);
    var right = this.editorUi.toolbar.addButton('geSprite-alignright', mxResources.get('rightAlign'),
        function() { graph.alignCells(mxConstants.ALIGN_RIGHT); }, stylePanel);

    var top = this.editorUi.toolbar.addButton('geSprite-aligntop', mxResources.get('topAlign'),
        function() { graph.alignCells(mxConstants.ALIGN_TOP); }, stylePanel);
    var middle = this.editorUi.toolbar.addButton('geSprite-alignmiddle', mxResources.get('middleAling'),
        function() { graph.alignCells(mxConstants.ALIGN_MIDDLE); }, stylePanel);
    var bottom = this.editorUi.toolbar.addButton('geSprite-alignbottom', mxResources.get('bottomAlign'),
...
ArrangePanel.prototype.addGeometry = function(container)
{
    ...
    var listener = mxUtils.bind(this, function(sender, evt, force)
    {
            ...
            rect.width != null && rect.height != null)
        {
            ...
            {
                width.value = rect.width + ((rect.width == '') ? '' : '');
            }
            
            if (force || document.activeElement != height)
            {
                height.value = rect.height + ((rect.height == '') ? '' : '');
            }
        }
        else
        {
            ...
            rect.x != null && rect.y != null)
        {
            ...
            {
                left.value = rect.x  + ((rect.x == '') ? '' : '');
            }
            
            if (force || document.activeElement != top)
            {
                top.value = rect.y + ((rect.y == '') ? '' : '');
            }
        }
        else
        {
...
ArrangePanel.prototype.addEdgeGeometry = function(container)
{
    ...
    span.style.marginTop = '0px';
    span.style.fontWeight = 'bold';
    mxUtils.write(span, mxResources.get('start'));
    ...
    span.style.marginTop = '0px';
    span.style.fontWeight = 'bold';
    mxUtils.write(span, mxResources.get('end'));
...
TextFormatPanel.prototype.addFont = function(container)
{
    ...
    var fontMenu = this.editorUi.toolbar.addMenu('PT Sans', mxResources.get('fontFamily'),
    ...
    var center = this.editorUi.toolbar.addButton('geSprite-center', mxResources.get('centerAlign'),
        (graph.cellEditor.isContentEditing()) ?
        function(evt)
        {
    ...
    var middle = this.editorUi.toolbar.addButton('geSprite-middle', mxResources.get('middleAlign'),
    ...
    var inputUpdate = this.installInputHandler(input, mxConstants.STYLE_FONTSIZE, Menus.prototype.defaultFontSize, 1, 999, '',
    function(fontSize)
    {
    ...
    var listener = mxUtils.bind(this, function(sender, evt, force)
    {
        ...
        if (force || document.activeElement != rightSpacing)
        {
            ...
            rightSpacing.value = (isNaN(tmp)) ? '' : tmp;
        }
                        ...
                        if (css != null)
                        {
                            ...
                            if (document.activeElement != input)
                            {
                                    ...
                                    pendingFontSize != null)
                                {
                                    ...
                                    node.style.fontSize = pendingFontSize;
                                    pendingFontSize = null;
                                }
                                else
                                {
                                    input.value = (isNaN(fontSize)) ? '' : fontSize;
                                }
...
StyleFormatPanel.prototype.init = function()
{
    ...
    if (!ss.containsLabel)
    {
        if (ss.containsImage && ss.vertices.length == 1 && (ss.style.shape == 'image' || ss.style.shape == 'bmrz') &&
        ...
        if (!ss.containsImage || ss.style.shape == 'image' || ss.style.shape == 'bmrz')
        {
...
StyleFormatPanel.prototype.addFill = function(container)
{
    ...
    var fillKey = (ss.style.shape == 'image' || ss.style.shape == 'bmrz') ? mxConstants.STYLE_IMAGE_BACKGROUND : mxConstants.STYLE_FILLCOLOR;
    var label   = (ss.style.shape == 'image' || ss.style.shape == 'bmrz') ? mxResources.get('background') : mxResources.get('fill');
    ...
    gradientPanel.style.display = (tmpColor != null && tmpColor != mxConstants.NONE &&
        ss.fill && ss.style.shape != 'image' && ss.style.shape != 'bmrz') ? '' : 'none';
...
StyleFormatPanel.prototype.addStroke = function(container)
{
    ...
    var strokeKey = (ss.style.shape == 'image' || ss.style.shape == 'bmrz') ? mxConstants.STYLE_IMAGE_BORDER : mxConstants.STYLE_STROKECOLOR;
    var label     = (ss.style.shape == 'image' || ss.style.shape == 'bmrz') ? mxResources.get('border') : mxResources.get('line');
    ...
    var edgeShape = this.editorUi.toolbar.addMenuFunctionInContainer(altStylePanel, 'geSprite-noarrow', mxResources.get('connection'), false, mxUtils.bind(this, function(menu)
    {
        this.editorUi.menus.styleChange(menu, '', [mxConstants.STYLE_SHAPE, mxConstants.STYLE_STARTSIZE, mxConstants.STYLE_ENDSIZE, 'width'], [null, null, null, null], 'geIcon geSprite geSprite-noarrow', null, true).setAttribute('title', mxResources.get('line'));
    ...
    var edgeStyle = this.editorUi.toolbar.addMenuFunctionInContainer(stylePanel2, 'geSprite-orthogonal', mxResources.get('waypoints'), false, mxUtils.bind(this, function(menu)
    {
        if (ss.style.shape != 'arrow')
        {
            ...
            this.editorUi.menus.edgeStyleChange(menu, '', [mxConstants.STYLE_EDGE, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE], [mxConstants.EDGESTYLE_ORTHOGONAL, null, null], 'geIcon geSprite geSprite-orthogonal', null, true).setAttribute('title', mxResources.get('orthogonal'));
            this.editorUi.menus.edgeStyleChange(menu, '', [mxConstants.STYLE_EDGE, mxConstants.STYLE_ELBOW, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE], [mxConstants.EDGESTYLE_ELBOW, null, null, null], 'geIcon geSprite geSprite-horizontalelbow', null, true).setAttribute('title', mxResources.get('simple'));
            this.editorUi.menus.edgeStyleChange(menu, '', [mxConstants.STYLE_EDGE, mxConstants.STYLE_ELBOW, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE], [mxConstants.EDGESTYLE_ELBOW, 'vertical', null, null], 'geIcon geSprite geSprite-verticalelbow', null, true).setAttribute('title', mxResources.get('simple'));
            this.editorUi.menus.edgeStyleChange(menu, '', [mxConstants.STYLE_EDGE, mxConstants.STYLE_ELBOW, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE], [mxConstants.EDGESTYLE_ISOMETRIC, null, null, null], 'geIcon geSprite geSprite-horizontalisometric', null, true).setAttribute('title', mxResources.get('isometric'));
            this.editorUi.menus.edgeStyleChange(menu, '', [mxConstants.STYLE_EDGE, mxConstants.STYLE_ELBOW, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE], [mxConstants.EDGESTYLE_ISOMETRIC, 'vertical', null, null], 'geIcon geSprite geSprite-verticalisometric', null, true).setAttribute('title', mxResources.get('isometric'));
    
            if (ss.style.shape == 'connector')
                this.editorUi.menus.edgeStyleChange(menu, '', [mxConstants.STYLE_EDGE, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE], [mxConstants.EDGESTYLE_ORTHOGONAL, '1', null], 'geIcon geSprite geSprite-curved', null, true).setAttribute('title', mxResources.get('curved'));
            
            this.editorUi.menus.edgeStyleChange(menu, '', [mxConstants.STYLE_EDGE, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE], [mxConstants.EDGESTYLE_ENTITY_RELATION, null, null], 'geIcon geSprite geSprite-entity', null, true).setAttribute('title', mxResources.get('entityRelation'));
        }
    }));
    ...
    var listener = mxUtils.bind(this, function(sender, evt, force)
    {
        ...
        if (es == mxConstants.EDGESTYLE_ORTHOGONAL && mxUtils.getValue(ss.style, mxConstants.STYLE_CURVED, null) == '1')
        {
        ...
        else if (es == mxConstants.EDGESTYLE_ENTITY_RELATION)
        {
            edgeStyleDiv.className = 'geSprite geSprite-entity';
        }
        else if (es == mxConstants.EDGESTYLE_ELBOW)
        {
            edgeStyleDiv.className = 'geSprite ' + ((mxUtils.getValue(ss.style,
                mxConstants.STYLE_ELBOW, null) == 'vertical') ? 'geSprite-verticalelbow' : 'geSprite-horizontalelbow');
        }
        else if (es == mxConstants.EDGESTYLE_ISOMETRIC)
        {
            edgeStyleDiv.className = 'geSprite ' + ((mxUtils.getValue(ss.style,
        ...
        if (force || document.activeElement != startSpacing)
        {
            ...
            startSpacing.value = (isNaN(tmp)) ? '' : tmp;
        }
        ...
        if (force || document.activeElement != perimeterSpacing)
        {
            ...
            perimeterSpacing.value = (isNaN(tmp)) ? '' : tmp;
        }
    });
...
StyleFormatPanel.prototype.addLineJumps = function(container)
{
    ...
    if (Graph.lineJumpsEnabled && ss.edges.length > 0 && ss.vertices.length == 0 && ss.lineJumps)
    {
        ...
        var styles = ['none', 'arc', 'gap', 'sharp', 'point'];
...
DiagramFormatPanel.prototype.addGridOption = function(container)
{
    ...
    input.value           = Math.max(graph.view.minGridSize, graph.getGridSize());
    ...
    {
        var value = parseInt(input.value);
        value = Math.max(graph.view.minGridSize, (isNaN(value)) ? graph.view.minGridSize : value);
