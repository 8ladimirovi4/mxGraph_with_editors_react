...
import { AJAX, isDefined, isNullOrEmpty, HELP, isFrame } from "./client";
...
const template = (function ()
{
    ...
    API.USER.refresh(webix.bind(function ()
    {
                ...
                var pingRoutine = function ()
                {
                    AJAX.get(API.FUNC.ping, null,
                        ...
                        function (xhr, err)
                        {
                            ...
                            console.log("common.errors.server_connection")
        ...
        {
            var sideMenu = [];
            sideMenu.push({ id: "home", value: 'template.pages.main', icon: "fas fa-laptop", href: "home" });
            //---->fix<------//
...
// @if !LINKMT
                sideMenu.push({
                    id: "exit", value: 'common.exit', icon: "fas fa-sign-out-alt", href: function ()
                    {
                        console.log('template.exit_confirm',function (res)
                        {
                            if (res === true)
                                API.USER.logout();
                        })
...
// @endif
        ...
        let notifyExecutionResult = function (item)
        {
            ...
            console.log("common.messages.command_execution_success")
        ...
        let buildTemplateLayout = function (complete)
        {
                        ...
                        [
                            {
                                ...
                                columns:
                                    [
                                        {
                                            id: "text", header: "common.name", fillspace: 0.5, tooltip: false,
                                        ...
                                        {
                                            id: "src", header: "common.source", fillspace: 0.5,
                                            template: "<a href='#link#' target='_blank'>#src#</a>"
                                        },
                                        {
                                            id: "ts", header: { text: "common.timestamp", css: "multiline" }, width: 210, tooltip: false,
                            ...
                            },
                            {
                                cols:
                                    [
                                        ...
                                        {
                                            view: "icon", type: "button", icon: "wxi-sync", css: "webix_transparent", width: 50, tooltip: "common.refresh", click: function ()
                                            {
            ...
            let notificationsWnd = webix.ui({
                ...
                close: true,
                resize: true,
                head: "template.notifications.title",
                body:
                {
                        ...
                        [
                            {
                                ...
                                onClick:
                                {
                                    "accept": function (evt, id, trg)
                                    {
                                        if (!isDefined(id))
                                            return;
                                            console.log("Квитировать выбранное уведомление", function (result)
                                            {
                                                if (result === true)
                                    ...
                                    "accept_list": function (evt, id, trg)
                                    {
                                        if (!isDefined(id))
                                            return;
                                            console.log("Квитировать все уведомления", function (result)
                                            {
                                                if (result === true)
            ...
            var timeTemplate = webix.template("<span class='fas fa-clock'></span>" + 'template.server_time' + ": #time#");
                ...
                body:
                {
                    ...
                    ready: function () 
                    {
                        let menuReady = webix.bind(function (menu)
                        {
                            ...
                            if (!isNullOrEmpty(window.location.pathname))
                            {
                                //let path = location.pathname.trimString('/');
                                let path = window.location.pathname.trimString('/');
                    ...
                    [
                        ...
                        demoView.buildView(),
                        {
                            ...
                            elements:
                                [
                                    {
                                        view: "toggle", type: "icon", id: "btnMenu", label:'common.menu', width: 100, value: 0,
                                    ...
                                    // @if WEBSCADA
                                    {
                                        ...
                                        tooltip: "template.pages.events", hidden: !context.showEventsIcon || !canViewEvents,
                                    ...
                                    },
                                    {
                                        ...
                                        tooltip: "template.alarms.title", hidden: !canViewAlarms,
                                        click: function (id, evt)
                                        {
                                            // reset badge
                                            this.config.badge = null;
                                    ...
                                    },
                                    {
                                        ...
                                        tooltip: "template.notifications.title", hidden: !canViewNotifications,
                                        click: function (id, evt)
                                        {
                                            // reset badge
                                            this.config.badge = null;
                                    ...
                                    // @endif
                                    {
                                        ...
                                        cols:
                                            [
                                                {
                                                    view: "icon", id: "connect_main", css: "main", icon: "state_stopped", tooltip: "template.main_server_state", width: 30, hidden: false
                                                },
                                                // @if WEBSCADA
                                                {
                                                    view: "icon", id: "connect_reserv", css: "reserv", icon: "state_stopped", tooltip: "template.reserv_server_state", width: 30, hidden: true
                                                }
                                                // @endif
                                            ]
                                    },
                                    // @if WEBSCADA
                                    {
                                        view: "button", type: "icon", icon: "fas fa-sign-out-alt", label: 'common.exit', tooltip:'common.exit', width: 100, hidden: false, click: function ()
                                        {
                                            console.log('template.exit_confirm',function (res){
                                                if (res === true)
                                                    API.USER.logout();
